...
import { Check, ArrowRight, Sparkles, HelpCircle, Shield, Globe, Zap, Cpu } from "lucide-react";
...
export default function PricingPage() {
  ...
  const plans = [
    ...
    {
      name: "Pro",
      description: "Infinite collaborative workspace for professional builders.",
  ...
  const featuresMatrix = [
    { category: "COLLABORATION & CANVAS", name: "Real-time Collaborative Editor", core: true, pro: true, enterprise: true },
    { category: "COLLABORATION & CANVAS", name: "Interactive Drawing Whiteboard", core: "Basic SVG drawing", pro: "Advanced tools + Custom exports", enterprise: "Tailored canvas APIs + Integrations" },
    { category: "COLLABORATION & CANVAS", name: "Active multiplayer cursors", core: true, pro: true, enterprise: true },
    { category: "COMMUNICATION", name: "High-definition Audio Calls", core: "Up to 3 players", pro: "Up to 50 players", enterprise: "Unlimited" },
    { category: "COMMUNICATION", name: "Video & Screen Sharing", core: false, pro: true, enterprise: true },
    { category: "SECURITY & DATA", name: "Vault Shared Storage", core: "1 GB", pro: "50 GB", enterprise: "Infinite" },
    { category: "SECURITY & DATA", name: "File Locking & Access controls", core: false, pro: true, enterprise: true },
    { category: "SECURITY & DATA", name: "SSO / SAML Integrations", core: false, pro: false, enterprise: true },
  ];
  ...
  return (
    <main className="flex-1 bg-[#0B0B0C] text-foreground py-20 px-6 md:px-8 border-b border-border/60 relative overflow-hidden">
      ...
      <meta name="description" content="Transparent, industrial-clean pricing for product teams seeking high-fidelity collaborative work suites." />
      ...
      <div className="mx-auto max-w-7xl flex flex-col items-center text-center mb-16 relative z-10">
        ...
        >
          An honest model for <span className="font-mono text-primary font-bold">real-time</span> teams.
        </motion.h1>
        <motion.p
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <button
            ...
            className={`relative rounded-full px-4 py-1.5 text-xs font-semibold transition-colors z-10 font-mono ${
              billingPeriod === "monthly" ? "text-white" : "text-muted-foreground hover:text-white"
            }`}
          >
          ...
          <button
            ...
            className={`relative rounded-full px-4 py-1.5 text-xs font-semibold transition-colors z-10 font-mono flex items-center gap-1.5 ${
              billingPeriod === "yearly" ? "text-white" : "text-muted-foreground hover:text-white"
            }`}
          >
        ...
        {plans.map((plan, index) => {
          ...
          return (
            <motion.div
              ...
              whileHover={{
                scale: 1.015,
                borderColor: plan.highlight ? "rgba(139, 92, 246, 0.4)" : "rgba(255, 255, 255, 0.15)",
              }}
            ...
            >
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="text-xl font-extrabold text-white font-mono">{plan.name}</h3>
                </div>
                ...
                <div className="flex items-baseline gap-1.5 mb-6.5 font-mono">
                  <span className="text-4xl md:text-5xl font-black text-white tracking-tight">
                    $
                    <AnimatedNumber value={billingPeriod === "monthly" ? plan.price.monthly : plan.price.yearly} />
                  </span>
                  <span className="text-xs text-muted-foreground">
                    /user/{billingPeriod === "monthly" ? "mo" : "mo billed annually"}
                  </span>
                </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-border/60">
                {featuresMatrix.map((feat, idx) => {
                  const isHeader = idx === 0 || feat.category !== featuresMatrix[idx - 1]?.category;
                  return (
                    <tr key={feat.name} className="hover:bg-[#161619]/40 transition-colors">
                      <td className="p-4">
                        {isHeader && (
                      ...
                      <td className="p-4 text-muted-foreground font-mono">
                        {typeof feat.core === "boolean" ? (
                          feat.core ? <Check className="size-4 text-emerald-400" /> : <span className="text-[10px] text-muted-foreground/30">&mdash;</span>
                        ) : (
                          feat.core
                        )}
                      </td>
                      <td className="p-4 text-white font-mono">
                        {typeof feat.pro === "boolean" ? (
                          feat.pro ? <Check className="size-4 text-primary" /> : <span className="text-[10px] text-muted-foreground/30">&mdash;</span>
                        ) : (
                          feat.pro
                        )}
                      </td>
                      <td className="p-4 text-white font-mono font-semibold">
                        {typeof feat.enterprise === "boolean" ? (
                          feat.enterprise ? <Check className="size-4 text-indigo-400" /> : <span className="text-[10px] text-muted-foreground/30">&mdash;</span>
                        ) : (
                          feat.enterprise
                        )}
                      </td>
      ...
      <div className="mx-auto max-w-4xl relative z-10 mt-28">
        <div className="text-center mb-12">
          <span className="text-[10px] font-extrabold font-mono text-primary tracking-widest uppercase">FAQ</span>
          <h2 className="text-2xl font-extrabold text-white mt-1">Frequently Asked Alignment</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {[
            ...
            <div key={idx} className="flex flex-col gap-2 p-5 rounded-lg border border-border bg-[#121214]">
              <div className="flex gap-2 items-start">
                <HelpCircle className="size-4 text-primary shrink-0 mt-0.5" />
                <h4 className="text-xs font-extrabold font-mono text-white uppercase">{faq.q}</h4>
              </div>
