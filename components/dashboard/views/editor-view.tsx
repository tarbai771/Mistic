...
export default function EditorView() {
  ...
  const docStructure = [
    { id: "q3_growth_sprint.mist", name: "q3_growth_sprint.mist", size: "4.2kb" },
    { id: "bundler_optimization.mist", name: "bundler_optimization.mist", size: "1.8kb" },
    { id: "system_design_specs.mist", name: "system_design_specs.mist", size: "12.1kb" },
  ];

  const comments = [
    { user: "Chloe", avatar: "C", color: "bg-emerald-500", text: "Love the layout here! Make sure to mention Framer Motion layoutId.", date: "Just now" },
    { user: "Devon", avatar: "D", color: "bg-indigo-500", text: "Is the database node setup fully persistent? Check Redis configs.", date: "1h ago" },
  ];
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-2 rounded text-xs font-mono transition-colors text-left relative ${
                    active ? "text-white" : "text-muted-foreground hover:text-white hover:bg-[#121214]/60"
                  }`}
                >
                  {active && (
                    <motion.span
      ...
      <div className="flex-1 flex flex-col justify-between overflow-hidden bg-[#0A0A0B]">
        ...
        <div className="flex items-center justify-between border-b border-border bg-[#0C0C0E]/90 px-6 py-3 select-none">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono font-semibold text-white">{activeDoc}</span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-[10px] text-muted-foreground font-mono">Live Cursor Sync</span>
          </div>
        </div>
        ...
        <div className="flex-1 p-8 overflow-y-auto font-mono text-xs leading-relaxed max-w-3xl mx-auto w-full select-text">
          ...
          <div className="relative border-l-2 border-amber-500/30 pl-4 mt-6 bg-amber-500/5 rounded p-3 border border-border/40 select-none">
            <span className="text-[9px] font-bold text-amber-400 block mb-2 flex items-center gap-1">
              <Users className="size-3 text-amber-500" /> Alex (Backend Developer) is contributing
            </span>
        ...
        <div className="p-4 border-t border-border bg-[#0C0C0E]/50 flex justify-between items-center select-none">
          <span className="text-[9px] text-muted-foreground font-mono flex items-center gap-1.5">
            <CheckCircle className="size-3 text-primary" /> Clean document specs parsed successfully.
          </span>
      ...
      <div className="w-64 border-l border-border bg-[#0C0C0E]/90 flex flex-col justify-between select-none">
        <div>
          ...
          <div className="p-4 border-b border-border/80 flex items-center justify-between">
            <span className="text-[10px] font-extrabold font-mono text-muted-foreground tracking-wider uppercase flex items-center gap-1.5">
              <MessageSquare className="size-3.5 text-primary" /> Inline Comments
            </span>
          ...
          <div className="p-4 flex flex-col gap-4">
            {comments.map((comment, index) => (
              <div key={index} className="flex gap-2.5 items-start">
                <span className={`size-5 rounded-full ${comment.color} flex items-center justify-center text-[9px] font-bold text-white shrink-0 font-mono`}>
                  {comment.avatar}
                </span>
                <div className="flex flex-col gap-1 text-[11px] leading-snug">
                  <div className="flex justify-between items-baseline">
                    ...
                    <span className="text-[9px] text-muted-foreground/60">{comment.date}</span>
                  </div>
